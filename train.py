...
from models.talklip import TalkLip, TalkLip_disc_qual, DISCEMO
...
class Talklipdata(object):
    ...
    def __getitem__(self, idx):
        ...
        emotion = self.getEmotion(sample)

        return inpImg, spectrogram, gtImg, trgt, volume, pickedimg, torch.from_numpy(imgs), torch.from_numpy(bbxs), emotion
...
## inpImg, spectrogram, gtImg, trgt, volume, pickedimg, torch.from_numpy(imgs), torch.from_numpy(bbxs)
def collate_fn(dataBatch):
    ...
    emotion = torch.cat([data[8] for data in dataBatch], dim=0)

    return inpBatch, audioBatch, audio_idx, gtBatch, targetBatch, padding_mask, pickedimg, videoBatch, bbxs, emotion
...
#####inpBatch, audioBatch, audio_idx, gtBatch, targetBatch, padding_mask, pickedimg, videoBatch, bbxs
    for epoch in range(args.n_epoch):
        losses = {'lip': 0, 'local_sync': 0, 'l1': 0, 'emo_loss': 0, 'prec_g': 0, 'disc_real_g': 0, 'disc_fake_g': 0}
        prog_bar = tqdm(enumerate(data_loader['train']))
        oom_count = 0
        for step, (
        inpim, spectrogram, audio_idx, gtim, ((trgt, prev_trg), tlen, ntoken), padding_mask, vidx, videos,
        bbxs, emotion) in prog_bar:
            ...
            try:
                ...
                if args.e_w > 0:
                    emo_loss = model['emo_disc'].percep_forward(syntim)
                ...
                pred = model['emo_disc'](syntim.detach())
                emo_disc_loss = nn.CrossEntropyLoss(pred, torch.argmax(emotion, dim=1))
                losses[emo_loss] += emo_disc_loss.item()
                emo_disc_loss.backward()
                optimizer['emo_opt'].step()
            ...
            if global_step == 1 or global_step % args.ckpt_interval == 0:
                ...
                save_checkpoint(model['disc'], optimizer['disc'], global_step, args.checkpoint_dir, epoch,
                                prefix='disc_')
...
def eval_model(test_data_loader, avhubert, criterion, global_step, device, model, disc, cont_w, recon_loss):
    ...
    for step, (
            x, spectrogram, audio_idx, gt, ((trgt, prev_trg), tlen, ntoken), padding_mask, vidx, videos,
            bbxs) in enumerate(
        (test_data_loader)):
        model.eval()
        disc.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--e_w', help='weight of emotion loss', default=1e-4, type=float)
    ...
    emoDisc = DISCEMO(args).to(device)
    ...
    emo_optimizer = optim.Adam([p for p in emoDisc.parameters() if p.requires_grad], lr=args.lr, betas=(0.5, 0.999))
    emo_scheduler = torch.optim.lr_scheduler.StepLR(emo_optimizer, args.steplr, gamma=0.1, last_epoch=-1)
    ...
    train(device, {'gen': imGen, 'disc': imDisc, 'emo_disc': emoDisc}, avhubert, criterion,
          {'train': train_data_loader, 'test': test_data_loader},
          {'gen': optimizer, 'disc': disc_optimizer, 'emo_opt': emo_optimizer, 'emo_sch': emo_scheduler}, args,
          global_step, logger)
