...
def fil_train(data):
    return (data.split('_')[3] == 'HI' or data.split('_')[3] == 'XX' or data.split('_')[3] == 'MD') and int(
        data.split('_')[0]) <= 1070
...
def fil_ref(data):
    return data.split('/')[4].startswith('B')
...
class Talklipdata(object):
    ...
    def __getitem__(self, idx):
        ...
        def process_sample(sample):
            ...
            trgt = self.label_proc(self.text_dict[words_text])
    ...
    for epoch in range(args.n_epoch):
        ...
        for step, (
            ...
            try:
                ...
                feats1, e1 = model['emoGen'](emotions, inpBatch)
                # print('EEEE1')
                # print(e1.shape)
                # print('xxx1')
                # print(x1.shape)
                ...
                #
                if args.e_w > 0:
                    loss1, x1emotion1 = get_emotions(x1e1, sample_len, model['emo_disc'], emotions, emo_loss_disc)
                    emo_loss = loss1
                    print('X1EMOTION1' + str(x1emotion1))
                    print('E1' + str(emotions))
                    emo_cont_loss = 0.0
                    #
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_epoch', help='number of epoch', default=1000000, type=int)
    parser.add_argument('--log_name', help='name of a log file', default='emotalklip', type=str)
    parser.add_argument('--ckpt_interval', help='The interval of saving a checkpoint', default=1000, type=int)
    ...
    # train_data_re_loader = DataLoader(train_dataset_re, batch_size=args.batch_size, shuffle=True,
    ...
    train_ref(device, {'gen': imGen, 'emoGen': emGen, 'decoder': decoder, 'disc': imDisc, 'emo_disc': emoDisc},
              avhubert,
              criterion,
              {'train': train_data_loader, 'test': train_data_loader},
