...
class Talklipdata(object):
    ...
    def __getitem__(self, idx):
        ...
        emo_num = random.randint(0, 5)
        while emo_num == self.emotion_dict[emotion_text]:
            emo_num = random.randint(0, 5)

        emotion = self.to_categorical(emo_num, num_classes=6)
...
def get_emotions(syntims, sample_lens, emo_disc, real_emotions):
    cnt = 0
    video_fragments = torch.split(syntims, sample_lens, dim=0)

    for i, fragment in enumerate(video_fragments):
        print(real_emotions[i])
        emotion = emo_disc(fragment)
        print(emotion)
        if torch.argmax(real_emotions[i], dim=0) == torch.argmax(emotion, dim=0):
            cnt += 1
    return cnt
...
def model_synt(test_data_loader, device, model, args):
    ...
    for inps, spectrogram, idAudio, gt, padding_mask, pickedimg, imgs, bbxs, emotion, sample_len, names in tqdm(
        ...
        emo_cnt += get_emotions(prediction, sample_len, model['emo_disc'], emotion)
        # if emo_pred_num == emo_real_num:
        #     emo_cnt += 1

    print('EMO ACC:' + str(emo_cnt / len(test_data_loader)))
...
if __name__ == "__main__":
    ...
    test_loader = DataLoader(test_dataset, batch_size=8, collate_fn=collate_fn, num_workers=6)  # hparams.batch_size, 4,
    ...
    emoDisc.load_state_dict(
        torch.load('/ssd2/m3lab/usrs/wy/TalkLip/emodisc_checkpoints/emo_disccheckpoint_step000062000.pth',
