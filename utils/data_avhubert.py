...
def collater_audio(audios, audio_size):
    ...
    elif len(audios[0].shape) == 3:
        collated_audios = collated_audios.permute((0, 3, 1, 2)).contiguous()
    else:
...
def emb_roi2im(pickedimg, bbxs, pred, device):
    trackid = 0
    processed_preds = []
    ...
    for i in range(len(pickedimg)):
        ...
        for j in range(len(idimg)):
            bbx = bbxs[i][idimg[j]]
            width, height = bbx[2] - bbx[0], bbx[3] - bbx[1]
            resize2ori = transforms.Resize([height, width])

            try:
                # Apply the transformation directly to pred
                resized_pred = resize2ori(pred[trackid + j] * 255.).permute(1, 2, 0)
                processed_preds.append(resized_pred.cpu().numpy())
            except Exception as e:
                print(f"Error processing bounding box {bbx}: {e}")
                import sys
                sys.exit()
...
def emb_roi2im_train(pickedimg, imgs, bbxs, pre, device, auio_shape):
    trackid = 0
    height, width, _ = imgs[0][0].shape
    for i in range(len(pickedimg)):
        idimg = pickedimg[i]
        imgs[i] = imgs[i].float().to(device)
        for j in range(len(idimg)):
            if len(bbxs[i]) < idimg[j]:
                bbx = bbxs[i][idimg[j] - auio_shape]
            else:
                bbx = bbxs[i][idimg[j]]
            if bbx[2] > width: bbx[2] = width
            if bbx[3] > height: bbx[3] = height
